...
def add_arguments(parser):
  ...
  #最新
  #python3 src/plot/ga_func2_plot.py --write_name 'loss_eva_ga_func_diff2_change_g100' --read_name ga_hf_loss_e20_p20_l10_c1_g100/ga_hf_pop_20 --model_path ga_hf_loss_e20_p20_l10_c1_g100/ga_hf_pop_20 --device 'cuda:1'
...
def all_pop_func_diff(args,models,inputdata_test,optimizer):
  ...
  for i in range(generation):
    ...
    #世代内の個体を一体づつ持ってくる
      ...
      sp_func2 =  np.sum(math.sqrt(sum((h_in_x)*(h_in_x)+(h_in_y)*(h_in_y))))
      tp_func2 =  np.sum(math.sqrt(sum((h_out_x)*(h_out_x)+(h_out_y)*(h_out_y))))
      #eva= nomal_eva(sp_var,tp_var)
      all_func_diff= sp_func2 + tp_func2
      ga_all_func_diff.append(all_func_diff)
      print(f'sp_var{sp_func2}----tp_var{tp_func2}----all_var{all_func_diff}')
    #移動平均の個数
    ave=10
    b=np.ones(ave)/ave
    #移動平均の描画
    ...
    plt.plot(gene[:(i+1)*pop],ga_all_func_diff,alpha= 1,label="generalizability Functional differentiation")
